...
class Bug(object):
    ...
    def __setattr__(self, attr, value):
        if attr == "_bug" or attr == "_copy":
            ...
            if self.id:
                ...
                raise BugException(
                    "Can not set status unless there is a bug id."
    ...
    def get_comments(self):
        ...
        return [
            Comment(bugsy=self._bugsy, **comments)
            for comments in res["bugs"][bug]["comments"]
        ]
    ...
    def add_comment(self, comment):
        ...
        if "id" in self._bug:
            self._bugsy.request(
                "bug/{}/comment".format(self._bug["id"]),
                method="POST",
    ...
    def get_attachments(self):
        ...
        return [
            Attachment(bugsy=self._bugsy, **attachments)
            for attachments in res["bugs"][bug]
        ]
    ...
    def add_attachment(self, attachment):
        ...
        self._bugsy.request(
            "bug/%s/attachment" % self._bug["id"], method="POST", json=output
        )
...
class Comment(object):
    ...
    def add_tags(self, tags):
        ...
        if not isinstance(tags, list):
            tags = [tags]
        self._bugsy.request(
            "bug/comment/%s/tags" % self._comment["id"],
            method="PUT",
            json={"add": tags},
        )
