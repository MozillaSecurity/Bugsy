...
class Bugsy(object):
    ...
    def __init__(
        ...
        if self.api_key:
            ...
            result = self.request(
                "login",
                headers={"X-Bugzilla-Login": username, "X-Bugzilla-Password": password},
            )
    ...
    def get(self, bug_number, include_fields=None):
        ...
        bug = self.request("bug/%s" % bug_number, params={"include_fields": fields})
    ...
    def put(self, bug):
        ...
        object initialisation and received back a token.
        ...
        if not self._have_auth:
            raise BugsyException(
                "Unfortunately you can't put bugs in Bugzilla without credentials"
            )
        ...
        if not bug.id:
            ...
            result = self.request("bug/%s" % bug.id, "PUT", json=bug.diff())
    ...
    def _handle_errors(self, response):
        if response.status_code >= 500:
            raise BugsyException(
                "We received a {0} error with the following: {1}".format(
                    response.status_code, response.text
                )
            )
        result = response.json()
        if (response.status_code > 399 and response.status_code < 500) or (
