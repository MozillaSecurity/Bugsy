...
def test_we_can_search_components():
    ...
    url_params = dict(component="Marionette")
    responses.add(
        responses.GET,
        rest_url("bug", **url_params),
        body=json.dumps(include_return),
    ...
    bugzilla = Bugsy()
    bugs = bugzilla.search_for.component("Marionette").search()
...
def test_we_can_search_products():
    ...
    url_params = dict(product="Testing")
    responses.add(
        responses.GET,
        rest_url("bug", **url_params),
        body=json.dumps(include_return),
    ...
    bugzilla = Bugsy()
    bugs = bugzilla.search_for.product("Testing").search()
...
def test_we_only_ask_for_the_include_fields_while_logged_in():
    ...
    bugs = bugzilla.search_for.include_fields("flags").search()
...
def test_that_we_can_search_for_a_specific_user():
    ...
    responses.add(
        responses.GET,
        rest_url("bug", assigned_to="dburns@mozilla.com"),
...
def test_we_can_search_with_change_history_field_throws_when_not_given_a_list():
    ...
    with pytest.raises(Exception) as e:
        bugzilla.search_for.change_history_fields("[Bug Creation]", "foo").timeframe(
            "2014-12-01", "2014-12-05"
        ).search()
    assert str(e.value) == "fields should be a list"
...
def test_we_can_search_with_change_history_field_gets_bugs():
    ...
    bugzilla = Bugsy()
    bugs = (
        bugzilla.search_for.change_history_fields(["[Bug creation]", "Alias"], "foo")
...
def test_we_can_handle_errors_coming_back_from_search():
    ...
    bugzilla = Bugsy()
    with pytest.raises(SearchException) as e:
        bugzilla.search_for.change_history_fields(
            ["[Bug Creation]", "Alias"], "foo"
        ).timeframe("2014-12-01", "2014-12-05").search()
    assert (
        str(e.value) == "Message: Can't use [Bug Creation] as a field name. Code: 108"
    )
