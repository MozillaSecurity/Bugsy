...
def test_we_cant_set_status_unless_there_is_a_bug_id():
    bug = Bug()
    with pytest.raises(BugException) as e:
        bug.status = "RESOLVED"
    assert (
        str(e.value)
...
def test_we_can_add_multiple_keywords_to_list(bug_return):
    ...
    keywords = bug.keywords
    assert sorted(["regression", "intermittent", "ateam-marionette-server"]) == sorted(
        keywords
    )
...
def test_we_can_add_multiple_emails_to_cc_list(bug_return):
    ...
    assert sorted(output["cc"]["add"]) == sorted(
        ["automatedtester@mozilla.com", "foobar@mozilla.com"]
    )
...
def test_we_can_add_remove_an_email_to_cc_list(bug_return):
    ...
    assert isinstance(output, dict)
    assert output == {
        "cc": {"add": ["automatedtester@mozilla.com"], "remove": ["dburns@mozilla.com"]}
    }
...
def test_we_throw_an_error_for_invalid_status_types(bug_return):
    ...
    with pytest.raises(BugException) as e:
        bug.status = "foo"
    assert str(e.value) == "Message: Invalid status type was used Code: None"
...
def test_we_cant_set_the_resolution_when_not_valid(bug_return):
    ...
    with pytest.raises(BugException) as e:
        bug.resolution = "FOO"
    assert str(e.value) == "Message: Invalid resolution type was used Code: None"
...
def test_we_raise_an_exception_if_commenting_on_a_bug_that_returns_an_error(bug_return):
    ...
    with pytest.raises(BugsyException) as e:
        bug.add_comment("I like sausages")
    assert str(e.value) == "Message: Bug 1017315 does not exist. Code: 101"
...
def test_we_cant_add_attachment_without_id(attachment_return):
    ...
    with pytest.raises(BugException) as e:
        bug.add_attachment(attachment)
    assert (
        str(e.value) == "Message: Cannot add an attachment without a bug id Code: None"
    )
